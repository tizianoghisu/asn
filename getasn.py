...
for line in lines:
    ...
    s1=line.split()[0]
    s2=line.split()[1]
    page="https://asn21.cineca.it/pubblico/miur/esito/"+s1+"%252F"+s2+"/2/2"
    ...
    if not ('Non risultano' in text):
        ...
        try:
            ...
            check=False
    if not check:
        page="https://asn21.cineca.it/pubblico/miur/esito/"+s1+"%252F"+s2+"/1/2"
        response=requests.get(page)
        text=response.text
        if not ('Non risultano' in text):
            check=True
...
for sec in secs[::-1]:
    s1=sec.split()[2]
    s2=sec.split()[3]
    p1="https://asn21.cineca.it/pubblico/miur/esito/"+s1+"%252F"+s2+"/1/2"
    p2="https://asn21.cineca.it/pubblico/miur/esito/"+s1+"%252F"+s2+"/2/2"
    f.write('\n')
